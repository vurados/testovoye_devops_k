...
def detect_host_type():
    ...
    Использует несколько эвристик: systemd-detect-virt, /.dockerenv, /proc/1/cgroup, DMI.
    """
    # 0) явные маркеры контейнера
    if os.path.exists('/.dockerenv') or os.path.exists('/run/.containerenv'):
        return 'container'
    if os.environ.get("container") in ("podman", "docker", "lxc"):
        return 'container'
    ...
    try:
        with open('/proc/1/cgroup', 'rt') as f:
            c = f.read().lower()
            if any(x in c for x in ('docker','kubepods','containerd','lxc','podman')):
                return 'container'
    except Exception:
        pass
    ...
    # 3) последнее приближение — считаем физическим
    return 'physical'

def main():
    # prometheus metric: один лейбл type с единственным значением 1
    host_type = detect_host_type()
    # host_info = Gauge('microservice_host_info', 'Host type info for microservice (labels: type)', ['type'])
    # host_info.labels(type=host_type).set(1)

    host_type_info = Info('Host type guess', 'Host type info for microservice')
